<?php
	...
	include '../api/config_post.php';

    $tech = 'post';//($_GET['type']) ? $_GET['type']:'Dashboard';	
	$title = 'Post';		
	$action = $api_url.'posts/adminpost.php';
	$data = json_decode(file_get_contents($action),true);
	$get_data_table_str = getDataTable($post_config,$data['rows'],$tech,'view',$con);
